    ...
    require_once("../libraries/password_compatibility_library.php");
		...
		if (empty($_POST['firstname'])){
			...
			require_once ("../config/conexion.php");//Contiene funcion que conecta a la base de datos
                ...
                $cedula_u = mysqli_real_escape_string($con,(strip_tags($_POST["cedula_u"],ENT_QUOTES)))
                    ;
                ...
                $sql = "SELECT * FROM users WHERE user_name = '" . $user_name . "' OR cedula_u ='" . $cedula_u . "' ;";

                $query_check_user_name = mysqli_query($con,$sql);
				$query_check_user=mysqli_num_rows($query_check_user_name);
                if ($query_check_user == 1) {
                    $errors[] = "Lo sentimos ,el nombre de usuario ó la cedula ya estÃ¡ en uso.";
                } else {
					// write new user's data into database
                    $sql = "INSERT INTO users (cedula_u, firstname, lastname, user_name, user_password_hash, user_email, date_added)
                            VALUES('".$cedula_u."','".$firstname."','".$lastname."','" . $user_name . "', '" . $user_password_hash . "', '" . $user_email . "','".$date_added."');";
