...
class PDF extends PDF_Rotate {
    ...
    var $aligns;
          ...
          function Header() {                         
           ...
           //  $this->Cell(195, 5,utf8_decode( get_row('perfil','direccion', 'id_perfil', 1)),0,5, 'R',0);                                
    ...
    while ($row = mysqli_fetch_array($resultado)) {        
        ...
        $pdf->Cell(66, 6, utf8_decode('Fecha: ' . strtoupper($row[6])),1,0, 'L',true); ///FECHA (X,Y)
        $pdf->Cell(67, 6, utf8_decode('Factura N° ' . strtoupper($row[14])),1,0, 'L',true); //Numero de factura
    ...
    while ($row = mysqli_fetch_array($resultado)) {
        ...
        $sub2 = (number_format(($total_venta),2,',','.')); 
        ...
        $pdf->SetY($posiciony);
        $pdf->SetX(130);
        $pdf->multiCell(50, 5, utf8_decode($sub2),0,0);
   ...
   $pdf->Text(5, 250, utf8_decode('Nro. de Items: ' . $total_items ), 0, 'C', 0); ////SUBTOTAL (X,Y) 
    ...
    $pdf->Output('Factura_Venta_'.$_GET['id'].'.pdf','I');
?>