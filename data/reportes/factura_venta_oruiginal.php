...
class PDF extends PDF_Rotate {
    ...
    var $aligns;
          ...
          function Header() {                         
            ...
            // $this->Cell(195, 8, get_row('perfil','nombre_empresa', 'id_perfil', 1), 0,1, 'C',0);                                
            ...
            // $this->Cell(110, 5,utf8_decode( get_row('perfil','ciudad', 'id_perfil', 1)." ".get_row('perfil','estado', 'id_perfil', 1)),0,5, 'R',0);   

            // $this->Cell(180, 5, utf8_decode("Teléfono: ").utf8_decode(get_row('perfil','telefono', 'id_perfil', 1)),0,1, 'C',0);                                
    ...
    $resultado = mysqli_query($con,"SELECT C.nombres_completos, C.identificacion, C.direccion, C.telefono2, C.ciudad,c.direccion, F.fecha_actual, F.forma_pago,F.subtotal, F.tarifa0, F.tarifa, F.iva_venta, F.descuento_venta, F.total_venta, F.serie from facturas F, clientes C WHERE F.id_cliente = C.id AND F.id = '$_GET[id]'");    
    ...
    while ($row = mysqli_fetch_array($resultado)) {
        ...
        $pdf->SetY($posiciony);
        $pdf->SetX(45);
        $pdf->multiCell(40, 5, utf8_decode(substr($descripcion, 0, $caracteres).'...'),0,1);
        ...
        $pdf->SetY($posiciony);
        $pdf->SetX(130);
        $pdf->multiCell(50, 5, utf8_decode($sub3),0,0);
   ...
   $pdf->Text(5, 250, utf8_decode('Nro. de Items: ' . $total_items ), 0, 'C', 0); ////SUBTOTAL (X,Y) 
    ...
    $pdf->Output('factura_'.$_GET['id'].'.pdf','I');
?>